...
public class MapGimic_6_Snake : AbstractMapGimic
{
    // 보라색 (Purple)
    private readonly Color snakeColor = new Color(0.6f, 0f, 0.8f);
    ...
    public override void OnGimicStart()
    {
        ...
        if (mapManager != null)
        {
            mapManager.SetScreenColor(snakeColor);

            if (MatchResultStore.myPlayerNumber == 1)
            {
                P2PMessageSender.SendMessage(MapGimicBuilder.BuildScreen_Color(snakeColor));
                StartCoroutine(Co_SnakeSequence());
            }
        }
    }
    ...
    public override void OnGimicEnd()
    {
        ...
        // 2. [추가] 2초 뒤에 남아있는 경고판(Gimic6_Entity) 삭제
    ...
    private IEnumerator Co_SnakeSequence()
    {
        // 1. 연출
        SendTween(0.2f, 0.4f);
            ...
            if (point_LeftBottom != null && point_RightUp != null)
            {
                ...
                if (validPosFound)
                {
                    ...
                    P2PMessageSender.SendMessage(
                        MapGimicBuilder.BuildSnake_Spawn(mapManager.GetMapGimicIndex(), spawnPos.x, spawnPos.y, spawnPos.z)
                    );
                }
            }
        }
    ...
    private IEnumerator Co_CleanupEntities()
    {
        ...
        // 씬에 있는 모든 Gimic6_Entity를 찾아서 삭제
        Gimic6_Entity[] entities = FindObjectsOfType<Gimic6_Entity>();
